...
public class ACO extends Algorithm {
	...
	public ACO() {
		...
		colonySize = 50;
		walkNumber = 200;
		double firstInputSolutionValue = 1195.0;
	...
	private void initialize() {
		...
		if (this.initialIndividual != null) {
			...
			initialAnt = 
					(Ant) new Ant(initialIndividual.getPersonWorkLists());
		...
		for (int i = 0; i < indexes.length; i++) {
			...
			for (int j = i+1; j < indexes.length; j++) {
				...
				//HEURISTICS
				double activityDuration = initialAnt.getPersonWorkLists().
						get(i/subProjectNumber).
						get(j/subProjectActivities).
						getWorkLoad();
				
				/**
				 * IF SHORT ACTIVITY DURATION PRIORITIZED:
				 * shortDurationPrioritized = 1.0/activityDuration, β > 1
				 * IF LONG ACTIVITY DURATION PRIORITIZED:
				 * shortDurationPrioritized = 1.0/activityDuration, β < 1
				 * If β > 1, high pressure on heuristics that may not be sufficient!
				 * The user should be aware of that.
				 * @author Karlo Knezevic, karlo.knezevic@fer.hr
				 */
				
				//SHORT ACTIVITY DURATION PRIORITIZED
				double shortDurationPrioritized = 
						activityDuration>0 ? 1.0/activityDuration : 0;

				heuristics[i][j] = Math.pow(shortDurationPrioritized, β);
